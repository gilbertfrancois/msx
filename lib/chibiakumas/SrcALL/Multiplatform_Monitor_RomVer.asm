...
;	call Monitor
	...
	ifndef Monitor_NextMonitorPos_2Byte 
		ifdef BuildCPC
Monitor_NextMonitorPos_2Byte equ &CFF0
		endif
		ifdef BuildGBC
MonitorRam equ &DF00
Monitor_NextMonitorPos_2Byte	equ MonitorRam+2
		endif
		ifdef BuildGMB
MonitorRam equ &DF00
Monitor_NextMonitorPos_2Byte	equ MonitorRam+2
...
;This is a dirty trick to check if interrupts are enabled or not 
	ifdef Monitor_Full
		...
		push af
			call MonitorEICheck
			ld a,c
			ld (Monitor_EI_Reenable_2byte),a	;DI /EI at end of monitor
			ld a,b
			ld (Monitor_Special),a			;letter D/E for visible string
			ld a,&c9
			ld (Monitor_EI_Reenable_2byte+1),a
		pop af
		pop bc
		di
	endif
...
;	ld (MonitorStackRestore_Plus2-2),sp	;We need to backup the true stack pointer, we're going to be messin' around!
	...
	push hl
	ld (Monitor_NextMonitorPos_2Byte),sp	;Store the current stack pointer, so we know where the legit data is.
	...
	ld hl,Monitor_NextMonitorPos_2Byte	;Pointer to the pushed values - we use this to read them out the stack.
	ld e,(hl)
	inc hl
	ld d,(hl)
	ld hl,Monitor_Text 
...
;	ld sp,&0000 MonitorStackRestore_Plus2
	...
	ifdef Monitor_Full				;SelfModifying code will change this to EI or DI depending if interrputs are needed
		call Monitor_EI_Reenable_2byte
	endif
	ret
